...
from core.models import Feed, Media, Member, Profile
...
def mid_to_url(mid):
    mid_int = str(mid)[::-1]
    size = (
        int(len(mid_int) // 7) if len(mid_int) % 7 == 0 else int(len(mid_int) // 7) + 1
    )
    result = []
    for i in range(size):
...
def standardize_date(created_at: str) -> datetime:
    if "刚刚" in created_at:
        return datetime.now()
    if "分钟" in created_at:
        num_minutes = int(created_at[: created_at.find("分钟")])
        delta = timedelta(minutes=num_minutes)
        return datetime.now() - delta
    if "小时" in created_at:
        num_hours = int(created_at[: created_at.find("小时")])
...
def save_content(user: Member, post: WeiboPost, version: int = 1) -> (Feed, bool):
    ...
    weibo = Feed.objects.create(
        author=post.author,
        link=post.url,
        created_at=post.created_at(version),
        title=post.text,
        user=user,
        type="weibo",
        metadata=post.status,
        status_id=post.id,
    )
...
def save_contents():
    ...
    for post in posts:
        ...
        user = get_or_create_user(
            post.user["idstr"], post.author, post.user["avatar_hd"]
        )
